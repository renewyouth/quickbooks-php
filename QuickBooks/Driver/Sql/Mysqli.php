...
class QuickBooks_Driver_Sql_Mysqli extends QuickBooks_Driver_Sql
{
    ...
    protected function _primary($table)
    {
    ...
    protected function checkTable($table)
    {
        try {
            $sql = "DESCRIBE " . $table . "";
            return $this->_query($sql, $errnum, $errmsg)->fetch_row() > 0;
        } catch (Exception $e) {
            return false;
        }
    }

    private function checkIndex($table, $index, $key)
    {
        try {
            $sql = "SHOW INDEX FROM " . $table . "  WHERE Seq_in_index =  " . $index . "  AND Column_name = '" . $key . "'";
            return $this->_query($sql, $errnum, $errmsg)->fetch_row() > 0;
        } catch (Exception $e) {
            return false;
        }
    ...
    private function checkKeys($table, $key)
    {
        $isExist = false;
        if (!is_array($key)){
            $key = [$key];
        ...
        foreach ($key as $k) {
            $count ++ ;
            $isExist = $this->checkIndex($table, $count, $k);
            if(!$isExist){
                break;
            }
        }
    ...
    protected function dropPrimary($name)
    {
        return $this->checkTable($name) ? " DROP PRIMARY KEY," : "";
    }
    ...
    protected function _generateCreateTable($name, $arr, $primary = array(), $keys = array(), $uniques = array(), $if_not_exists = true)
    {
        ...
        if (is_array($primary)) {
            ...
            $arr_sql[] = 'ALTER TABLE ' . $name . $this->dropPrimary($name) . ' ADD PRIMARY KEY ( ' . implode(', ', $primary) . ' ) ';
            //$arr_sql[] = 'ALTER TABLE ' . $name . ' ADD PRIMARY KEY ( ' . implode(', ', $primary) . ' ) ';
        } else if ($primary) {

            $arr_sql[] = 'ALTER TABLE ' . $name . $this->dropPrimary($name) . ' ADD PRIMARY KEY (' . $primary . '); ';
